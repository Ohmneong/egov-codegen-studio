...
>
> **여러 테이블을 한 번에?** 한 파일에 `CREATE TABLE`을 여러 개 적으면 **테이블마다 한 세트씩** 만들어집니다.
...
4. `[미리보기]`를 누르면 **만들지 않고** 어떤 파일이 어디에 생길지(이미 있는 파일은 "덮어씀") 먼저 보여줍니다.
5. `[생성]`을 누르면 오른쪽에 만들어진 파일 목록과 접속 주소가 뜹니다. (이미 있는 파일이 있으면 덮어쓸지 물어봅니다.)

> 맨 위 **프로파일** 바에서 프로젝트별 설정을 `[현재 설정 저장]`으로 저장해 두고, 다음에 콤보박스 + `[불러오기]`로 바로 전환할 수 있습니다.
