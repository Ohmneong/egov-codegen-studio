...
## 📖 문서 안내
- **[SESSION-GUIDE.md](./SESSION-GUIDE.md)** — **새 세션 진입점**. 이어 작업할 때 시작 절차·현재 스냅샷·운영 함정 (가장 먼저)
- **[HANDOVER.md](./HANDOVER.md)** — 프로젝트를 이어받는 분용. 전체 그림·작업 경과·설계 결정·남은 일
...
```powershell
java -jar dist\egov-codegen-studio.jar --ddl sample\sample.sql --config gen.properties
```
